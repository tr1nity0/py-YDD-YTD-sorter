...
def main():
    ...
    # --- Index all .ytd and .ydd files in the working directory ---
    all_ytds: dict[str, str] = {}
    all_ydds: dict[str, str] = {}
    ...
    print(f"Found {len(all_ytds)} .ytd and {len(all_ydds)} .ydd file(s) in directory.\n")
    ...
    for ydd in ydd_bases:
        ...
        # --- Copy the YDD itself ---
        ydd_key = ydd.lower()
        if ydd_key in all_ydds:
            ydd_file = all_ydds[ydd_key]
            src = os.path.join(work_dir, ydd_file)
            dst = os.path.join(sorted_path, ydd_file)
            if os.path.isfile(src):
                os.makedirs(sorted_path, exist_ok=True)
                if os.path.exists(dst):
                    print(f"  [skip]        {ydd_file}  (already exists)")
                    skipped += 1
                else:
                    shutil.copy2(src, dst)
                    print(f"  [copied]      {ydd_file}")
                    copied += 1
